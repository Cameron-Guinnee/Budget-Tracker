...
def wordcloud_tab(df: pd.DataFrame): 
    ...
    if income_toggle:
        ...
        weights = pd.Series(memo_grouped_data['Price'].values, index=memo_grouped_data['Memo']).to_dict()
        cloud = WordCloud(background_color='rgba(255, 255, 255, 0)', mode='RGBA', scale=10, colormap='tab20', width=800).generate_from_frequencies(weights)
